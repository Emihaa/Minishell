...
t_node *create_node(t_node *root, t_token *token, t_arena *arena)
{
	...
	new_node->right = NULL;
	new_node->root = root;
	if (token->type == PIPE) //@TODO: this is for printing and SHOULD BE removed later
	{
		new_node->token.u_data.string = ft_itoa(value++);
		 new_node->token.string_len = ft_strlen(new_node->token.u_data.string);
	}
	return (new_node);
}
